...
### Отсутствие protoc-gen-go
Если скрипт сообщает, что `protoc-gen-go` не найден, выполните команду:
```bash
go install google.golang.org/protobuf/cmd/protoc-gen-go@latest
```
И убедитесь, что папка `$HOME/go/bin` (или `%USERPROFILE%\go\bin` на Windows) добавлена в вашу переменную PATH.
